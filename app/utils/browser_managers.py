...
class ChromeManagerExample(SeleniumManager):
    """Менеджер для хрома"""

    def __init__(self, driver_path: str, driver: Type[WebDriver], headless: bool) -> None:
        super().__init__(driver_path, driver, headless)


if __name__ == "__main__":
    # manager = SeleniumManager(str(BASE_DIR / 'drivers/geckodriver.exe'))
    # manager.default_driver.get("https://google.com")
    # time.sleep(3)
    # manager.close_and_quit()
    manager = ChromeManagerExample(str(BASE_DIR / 'drivers/chromedriver.exe'), webdriver.Chrome, False)
    manager.default_driver.get("https://google.com")
    time.sleep(3)
    manager.close_and_quit()
